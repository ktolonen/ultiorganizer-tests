...
final class ScoresheethistoryFunctionsLibTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        // (see GameFunctionsLibTest). ScoresheetHistoryCount()/List()/All() build the
    ...
    protected function tearDown(): void
    {
        ...
        // ScoresheetHistoryRestore() rebuilds uo_played via GameRemoveAllPlayers()/
        ...
        // ScoresheetHistoryRestore() now calls GameRemoveAllGameEvents() before
        ...
        // ScoresheetHistoryRestore() ends by calling GameSetForfeit(), which always
    ...
    {
        $id = (int) ScoresheetHistoryRecord(700, 'result', 'update', ['home' => 15, 'away' => 11]);
        $this->assertGreaterThan(0, $id);

        $row = DBQueryToRow(
            "SELECT game, user_id, ip, target, action, detail, has_snapshot, snapshot
             FROM uo_scoresheet_history WHERE history_id=$id",
        );
    ...
    public function testSnapshotRowAlsoOmitsTheAddressWhileVisitorLoggingIsDisabled(): void
    {
        ...
        $id = (int) ScoresheetHistorySnapshotIfNeeded(700);
        $this->assertGreaterThan(0, $id);
        $row = DBQueryToRow("SELECT ip, has_snapshot FROM uo_scoresheet_history WHERE history_id=$id");
    ...
    public function testAddressIsStillRecordedWhenVisitorLoggingIsEnabled(): void
    {
        ...
        try {
            $id = (int) ScoresheetHistoryRecord(700, 'result', 'update', ['home' => 2, 'away' => 1]);
            $row = DBQueryToRow("SELECT ip FROM uo_scoresheet_history WHERE history_id=$id");
            $this->assertSame('203.0.113.7', $row['ip']);

            $snapshotId = (int) ScoresheetHistorySnapshotIfNeeded(700);
            $snapshotRow = DBQueryToRow("SELECT ip FROM uo_scoresheet_history WHERE history_id=$snapshotId");
    ...
    {
        unset($_SESSION['uid']);
        $id = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 5]);
        $row = DBQueryToRow("SELECT user_id FROM uo_scoresheet_history WHERE history_id=$id");
        $this->assertSame('unknown', $row['user_id']);
    }

    public function testRecordRejectsInvalidGameId(): void
    {
        $this->assertFalse(ScoresheetHistoryRecord(0, 'goal', 'add', []));
    }
    ...
    public function testRecordIsSuppressedWhileTheSuppressionFlagIsSet(): void
    {
        ...
        try {
            ...
            ScoresheetHistorySuppressed(false);
        }
        $count = (int) DBQueryToValue("SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700");
    ...
    public function testRecordDeniesASessionWithNoneOfTheFourRights(): void
    {
        ...
        // one combination ScoresheetHistoryAuthorized() must refuse.
    ...
    public function testAccreditationOnlyRightCannotWriteHistory(): void
    {
        ...
        $_SESSION['uid'] = 'anonymous';
        try {
            $this->assertFalse(ScoresheetHistoryRecord(700, 'result', 'update', ['home' => 9, 'away' => 9]));
            $this->assertFalse(ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 99]));
            $this->assertFalse(ScoresheetHistoryRecord(700, 'forfeit', 'update', ['forfeit' => 'home']));
    ...
    public function testRecordSucceedsForAMediaOnlyRight(): void
    {
        ...
        try {
            $id = (int) ScoresheetHistoryRecord(700, 'mediaevent', 'add', ['url' => 1]);
        } finally {
            $_SESSION['uid'] = 'testuser';
            $_SESSION['userproperties']['userrole'] = ['superadmin' => true];
        }
    ...
    public function testRecordDeniesAMediaOnlyRightForANonMediaTarget(): void
    {
        ...
        // true for any logged-in session -- so ScoresheetHistoryAuthorized() must
        ...
        $count = (int) DBQueryToValue("SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND target='result'");
        $this->assertSame(0, $count);
    }
    ...
    public function testRecordDeniesAMediaOnlyRightForAGoalTarget(): void
    {
        ...
        $count = (int) DBQueryToValue("SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND target='goal'");
        $this->assertSame(0, $count);
    }
    ...
    public function testSnapshotIfNeededDeniesAMediaOnlyRight(): void
    {
        ...
        // from ScoresheetHistoryBuildSnapshot()), so hasAddMediaRight() must not
        // be accepted here at all -- ScoresheetHistorySnapshotIfNeeded() passes no
        // $target to ScoresheetHistoryAuthorized(), so the media branch can never
    ...
    {
        // ScoresheetHistoryWriteSnapshot() used to perform the uo_scoresheet_history
        // insert with none of ScoresheetHistorySnapshotIfNeeded()'s own guards
        // (disabled setting, suppression, $gameId, ScoresheetHistoryAuthorized())
        // applied to it directly -- its only caller was the CacheRemember()
        // closure inside ScoresheetHistorySnapshotIfNeeded() itself. It is now
        ...
        // every guard above.
        $this->assertFalse(function_exists('ScoresheetHistoryWriteSnapshot'));
    }

    public function testBuildSnapshotCapturesGameScalarsGoalsAndPlayers(): void
    {
        $snapshot = ScoresheetHistoryBuildSnapshot(700);
    ...
    {
        $first = (int) ScoresheetHistorySnapshotIfNeeded(700);
        $this->assertGreaterThan(0, $first);

        // The second call is memoized: it returns the same id and writes no
        // second row.
        $this->assertSame($first, (int) ScoresheetHistorySnapshotIfNeeded(700));

        $rows = DBQueryToArray(
            "SELECT target, action, has_snapshot, snapshot FROM uo_scoresheet_history WHERE game=700",
        );
    ...
    public function testGameSetResultRecordsTheFinalScore(): void
    {
        ...
        $row = DBQueryToRow(
            "SELECT target, action, detail FROM uo_scoresheet_history
    ...
    public function testGameAddScoreEntryRecordsOneGoalRowPerPoint(): void
    {
        ...
        $rows = DBQueryToArray(
            "SELECT action, detail FROM uo_scoresheet_history WHERE game=701 AND target='goal'",
        );
        $this->assertCount(1, $rows);
    ...
    public function testGameRemoveAllScoresSnapshotsBeforeClearingAndRecordsTheCount(): void
    {
        ...
        $snapshot = DBQueryToRow(
            "SELECT snapshot FROM uo_scoresheet_history
    ...
    public function testGameSetScoreSheetKeeperRecordsTheOfficialName(): void
    {
        ...
        $row = DBQueryToRow(
            "SELECT target, detail FROM uo_scoresheet_history
    ...
    public function testGameAddNewPlayerRecordsExactlyOnePlayedAddRow(): void
    {
        ...
        $count = (int) DBQueryToValue(
            "SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=701 AND target='played' AND action='add'",
        );
        $this->assertSame(1, $count);
    ...
    public function testListReturnsNewestFirstAndOmitsTheSnapshotPayload(): void
    {
        ...
        ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 2, 'score' => '1-1']);

        $rows = ScoresheetHistoryList(700);
    ...
    public function testCountMatchesTheNumberOfRecordedRows(): void
    {
        ...
        $this->assertSame(2, ScoresheetHistoryCount(700));
    }
    ...
    public function testListDeniesAUserWithoutGameEditRights(): void
    {
        ...
        $_SESSION['uid'] = 'anonymous';
        try {
            $this->assertSame([], ScoresheetHistoryList(700));
    ...
    {
        $id = (int) ScoresheetHistorySnapshotIfNeeded(700);
        $entry = ScoresheetHistoryEntry($id);
    ...
    public function testSeasonSummaryDeniesUsersWithoutEventRights(): void
    {
        ...
        $_SESSION['userproperties']['userrole'] = [];
        try {
            $this->assertSame([], SeasonScoresheetHistorySummary('HRN2026'));
            ...
            $this->assertNotSame([], SeasonScoresheetHistorySummary('HRN2026'));
        } finally {
            $_SESSION['userproperties']['userrole'] = ['superadmin' => true];
        }
    }
    ...
    public function testSeasonSummaryPicksTheLatestRowByHistoryIdWhenTimesTie(): void
    {
        ...
        $first = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 1]);
        $_SESSION['uid'] = 'later';
        $second = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 2]);
        $this->assertGreaterThan($first, $second);

        DBQuery("UPDATE uo_scoresheet_history SET time='2026-06-01 10:30:00' WHERE game=700");

        $rows = SeasonScoresheetHistorySummary('HRN2026');
        $this->assertCount(1, $rows);
    ...
    public function testSeasonSummaryMarksALastChangeOutsideTheScheduledDay(): void
    {
        ...
        $id = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 1]);

        DBQuery(sprintf(
            "UPDATE uo_scoresheet_history SET time='2026-06-01 23:30:00' WHERE history_id=%d",
            $id,
        ));
        $rows = SeasonScoresheetHistorySummary('HRN2026');
        $this->assertCount(1, $rows);
        $this->assertFalse((bool) $rows[0]['offday']);
        $this->assertSame([], SeasonScoresheetHistorySummary('HRN2026', ['offday' => true]));

        DBQuery(sprintf(
            "UPDATE uo_scoresheet_history SET time='2026-06-02 00:30:00' WHERE history_id=%d",
            $id,
        ));
        $rows = SeasonScoresheetHistorySummary('HRN2026');
        $this->assertCount(1, $rows);
        $this->assertTrue((bool) $rows[0]['offday']);
        $this->assertCount(1, SeasonScoresheetHistorySummary('HRN2026', ['offday' => true]));
    }
    ...
    {
        $timeout = ScoresheetHistoryFormatDetail([
            'target' => 'timeout',
            'action' => 'add',
        ...
        $defense = ScoresheetHistoryFormatDetail([
            'target' => 'defense',
            'action' => 'clear',
        ...
        $comment = ScoresheetHistoryFormatDetail([
            'target' => 'comment',
            'action' => 'remove',
        ...
        $forfeit = ScoresheetHistoryFormatDetail([
            'target' => 'forfeit',
            'action' => 'update',
        ...
        $spiritTimeout = ScoresheetHistoryFormatDetail([
        ...
        $mediaevent = ScoresheetHistoryFormatDetail([
            'target' => 'mediaevent',
            'action' => 'add',
        ...
        $gameeventStart = ScoresheetHistoryFormatDetail([
            'target' => 'gameevent',
            'action' => 'update',
        ...
        $gameeventCap = ScoresheetHistoryFormatDetail([
            'target' => 'gameevent',
            'action' => 'update',
    ...
    {
        $this->assertSame('Start game clock', ScoresheetHistoryFormatDetail([
            'target' => 'timer', 'action' => 'start', 'detail' => json_encode([]),
        ]));
        $this->assertSame('Pause game clock', ScoresheetHistoryFormatDetail([
            'target' => 'timer', 'action' => 'pause', 'detail' => json_encode([]),
        ]));
        $this->assertSame('Resume game clock', ScoresheetHistoryFormatDetail([
    ...
    {
        $ongoing = ScoresheetHistoryFormatDetail([
            'target' => 'result',
            'action' => 'update',
        ...
        $fromGoals = ScoresheetHistoryFormatDetail([
            'target' => 'result',
            'action' => 'update',
    ...
    public function testGameSetStartingTeamDistinguishesClearedFromAwayStarts(): void
    {
        ...
        $awayText = ScoresheetHistoryFormatDetail([
            'target' => 'gameevent',
        ...
        $clearedText = ScoresheetHistoryFormatDetail([
            'target' => 'gameevent',
    ...
    public function testRestorePutsBackTheGoalSequenceAndResult(): void
    {
        ...
        // every restore of this fixture -- see ScoresheetHistoryRestore().
    ...
    public function testRestoreWritesOneRestoreRowAndOneSnapshotNotAPerGoalTrail(): void
    {
        ...
        DBQuery("DELETE FROM uo_scoresheet_history WHERE game=700 AND has_snapshot=0");
        ScoresheetHistoryRestore($snapshotId);
        ...
        $goalRows = (int) DBQueryToValue(
            "SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND target='goal'",
        );
        $this->assertSame(0, $goalRows);

        $restoreRows = (int) DBQueryToValue(
            "SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND target='restore'",
        );
    ...
    {
        $id = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 1]);
        $result = ScoresheetHistoryRestore($id);
    ...
    public function testRestoreDeniesATeamAdminMissingTheAccreditationRightAndLeavesGameUntouched(): void
    {
        ...
        // not ScoresheetHistoryEntry()'s single hasEditGameEventsRight() check.
        $snapshotId = (int) ScoresheetHistorySnapshotIfNeeded(700);
    ...
    public function testRestoreRejectsATeamReassignmentButStillHonorsAPreV4SnapshotLackingTeamKeys(): void
    {
        ...
        $preV4Snapshot = ScoresheetHistoryBuildSnapshot(700);
        ...
        $v4SnapshotId = (int) ScoresheetHistorySnapshotIfNeeded(700, true);
        ...
        try {
            ...
            $mismatchResult = ScoresheetHistoryRestore($v4SnapshotId);
            ...
            // differ.
            $backCompatResult = ScoresheetHistoryRestore($preV4SnapshotId);
    ...
    public function testReadingAnEntryWithholdsASnapshotWhoseFixtureNoLongerMatches(): void
    {
        ...
        $snapshotId = (int) ScoresheetHistorySnapshotIfNeeded(700, true);
        $this->assertIsArray(ScoresheetHistoryEntry($snapshotId)['snapshot']);

        GameChangeHome(700);
        try {
            $entry = ScoresheetHistoryEntry($snapshotId);
    ...
    public function testBuildSnapshotCapturesTeamsAsFormatV4AndPreservesAnUnassignedSideAsNull(): void
    {
        ...
        try {
            $snapshot = ScoresheetHistoryBuildSnapshot(701);
            $this->assertSame(4, $snapshot['v']);
    ...
    public function testFormatDetailRendersRoleAssignmentsNotPlayerZero(): void
    {
        ...
        $captainText = ScoresheetHistoryFormatDetail([
            'target' => 'played',
        ...
        $spiritText = ScoresheetHistoryFormatDetail([
            'target' => 'played',
    ...
    public function testRestoreReproducesTheStartingOffence(): void
    {
        ...
        // 'offence', while ScoresheetHistoryFormatDetail() renders the same row's
        // audit label as "start" -- two different vocabularies. The replay in
        // ScoresheetHistoryRestore() must compare against the real column value,
    ...
    public function testKeeperHalftimeAndStartingTeamChangedInOneSaveAreUndoable(): void
    {
        ...
        $snapshotId = (int) DBQueryToValue(
            "SELECT history_id FROM uo_scoresheet_history
    ...
    public function testRestoreRemovesAPostSnapshotCapEventButLeavesMediaRowsAlone(): void
    {
        ...
        $snapshotId = (int) ScoresheetHistorySnapshotIfNeeded(701);
    ...
    {
        // admin/seasonscoresheethistory.php feeds a bare YYYY-MM-DD from <input
        // type='date'>, which MySQL widens to 00:00:00 -- a plain `<=` would
        // exclude every row recorded later on the chosen end date.
        $id = (int) ScoresheetHistoryRecord(700, 'goal', 'add', ['num' => 1]);
        DBQuery(sprintf(
            "UPDATE uo_scoresheet_history SET time='%s 23:00:00' WHERE history_id=%d",
            date('Y-m-d'),
            $id,
        ));

        $rows = SeasonScoresheetHistorySummary('HRN2026', ['to' => date('Y-m-d')]);
        $this->assertCount(1, $rows);
        $this->assertSame('700', (string) $rows[0]['game_id']);
    }
    ...
    public function testRestoreDoesNotRewriteTheGlobalJerseyOfATransferredPlayer(): void
    {
        ...
        try {
            $this->assertTrue(ScoresheetHistoryRestore($snapshotId)['restored']);
    ...
    {
        // C2: restore is an event-admin action (hasRestoreScoresheetHistoryRight()),
    ...
    public function testRestoreIsRefusedForATeamOrGameAdminWhoMayStillReadTheHistory(): void
    {
        ...
        try {
            ...
            $this->assertNotEmpty(ScoresheetHistoryList(701));
        } finally {
            $_SESSION['uid'] = 'testuser';
            $_SESSION['userproperties']['userrole'] = ['superadmin' => true];
            DBQuery(sprintf("DELETE FROM uo_played WHERE player=%d AND game=701", $playerId));
    ...
    public function testBuildSnapshotCapturesDefensesAndTimerFields(): void
    {
        ...
        // on time() at the moment ScoresheetHistoryBuildSnapshot() ran, only that
    ...
    public function testRestoreRoundTripsDefenseCountsAndTimerColumns(): void
    {
        ...
        // ScoresheetHistoryRestoreResult() now derives fresh from the captured
        // elapsed time instead of replaying verbatim (see C3 in
        // docs/scoresheet-history.md). isongoing=1 is needed too: GameTimerState()
    ...
    public function testRestoreLeavesDefensesUnchangedForAnOldFormatSnapshotButStillNullsTheTimer(): void
    {
        ...
        $result = ScoresheetHistoryRestore($historyId);
    ...
    public function testRestoreOfAnOlderSnapshotWithoutTimerElapsedStillReplaysTheEpochVerbatim(): void
    {
        ...
        // timer_elapsed key, so ScoresheetHistoryRestoreResult() must fall back to
    ...
    public function testRestoreOfARunningClockReproducesTheElapsedTimeAtCaptureNotTheWallClockDelayUntilRestore(): void
    {
        ...
        // ScoresheetHistorySnapshotIfNeeded() (so ScoresheetHistoryBuildSnapshot()
        ...
        $row = DBQueryToRow("SELECT snapshot FROM uo_scoresheet_history WHERE history_id=$snapshotId");
        ...
        DBQuery(sprintf(
            "UPDATE uo_scoresheet_history SET snapshot='%s' WHERE history_id=%d",
            DBEscapeString(json_encode($snapshot)),
            $snapshotId,
        ));
    ...
    public function testDeletingOwnNoteAfterLosingTheEditRightStillRecordsAnAuditRow(): void
    {
        ...
        try {
            ...
            $row = DBQueryToRow(
                "SELECT user_id, target, action FROM uo_scoresheet_history
            ...
            $this->assertSame(
                1,
                (int) DBQueryToValue("SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND has_snapshot=1"),
    ...
    public function testUpdatingOwnNoteAfterLosingTheEditRightStillRecordsAnAuditRow(): void
    {
        ...
        try {
            ...
            $snapshot = DBQueryToValue(
                "SELECT snapshot FROM uo_scoresheet_history WHERE game=700 AND has_snapshot=1 ORDER BY history_id ASC LIMIT 1",
            );
    ...
    public function testForceCapturedSnapshotAndRestoreAuditRowSurviveWhileRecordingIsDisabled(): void
    {
        ...
        try {
            ...
            // ScoresheetHistoryRestore()'s own pre-restore capture.
            $snapshotId = (int) ScoresheetHistorySnapshotIfNeeded(700, true);
            $this->assertGreaterThan(0, $snapshotId);
            ...
            // and its restore audit row must both survive DisableScoresheetHistory=true.
            $snapshots = (int) DBQueryToValue(
                "SELECT COUNT(*) FROM uo_scoresheet_history WHERE game=700 AND has_snapshot=1",
            );
            $this->assertSame(2, $snapshots);
