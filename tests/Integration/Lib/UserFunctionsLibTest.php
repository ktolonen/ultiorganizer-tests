...
final class UserFunctionsLibTest extends TestCase
{
    ...
    {
        // uo_scoresheet_history and uo_event_log attribute a row by login name, so a
        ...
        DBQuery("INSERT INTO uo_scoresheet_history (game, user_id, ip, source, target, action, detail)
        ...
        try {
            ...
            $this->assertSame(0, (int) DBQueryToValue(
                "SELECT COUNT(*) FROM uo_scoresheet_history WHERE user_id='testuser_ren'",
            ), 'no history may be left under the old login');
            $this->assertSame(1, (int) DBQueryToValue(
                "SELECT COUNT(*) FROM uo_scoresheet_history WHERE user_id='testuser_ren2'",
            ));
            $this->assertSame(1, (int) DBQueryToValue(
                "SELECT COUNT(*) FROM uo_event_log WHERE user_id='testuser_ren2'",
            ));
        } finally {
            DBQuery("DELETE FROM uo_scoresheet_history WHERE user_id IN ('testuser_ren', 'testuser_ren2')");
