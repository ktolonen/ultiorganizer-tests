...
final class PrivacyFunctionsLibTest extends TestCase
{
    ...
    // ===== uo_scoresheet_history coverage =====
    //
    // Fixture game 700 (baseline.sql) is reused as the FK target for every
    // uo_scoresheet_history row inserted below; each test deletes exactly the
    // history_id(s) and player_id(s) it created, never a blanket delete by
    // game, to avoid colliding with ScoresheethistoryFunctionsLibTest's own use of
    // game 700/701.
    ...
    private function insertScoresheetHistorySnapshotRow(?array $snapshot, string $userId = 'admin', string $ip = '203.0.113.9'): int
    {
        $json = $snapshot === null ? null : json_encode($snapshot, JSON_UNESCAPED_UNICODE);
        if ($snapshot === null) {
            return (int) DBQueryInsert(sprintf(
                "INSERT INTO uo_scoresheet_history (game, user_id, ip, source, target, action, has_snapshot, snapshot, detail)
    ...
    /**
     * Minimal snapshot matching ScoresheetHistoryBuildSnapshot()'s shape, with only
    ...
    private function cleanupScoresheetHistoryRows(array $historyIds): void
    {
        if (empty($historyIds)) {
            return;
        }
        $idList = implode(',', array_map('intval', $historyIds));
        DBQuery("DELETE FROM uo_scoresheet_history WHERE history_id IN ($idList)");
    }
    ...
    public function testPrivacyAnonymizePlayerRewritesSnapshotNameFieldsAndLeavesOtherFreeTextAlone(): void
    {
        ...
        $historyId = $this->insertScoresheetHistorySnapshotRow($snapshot);
        try {
            self::flushQueryCaches();
            $result = PrivacyAnonymizePlayer($playerId, 'admin');
            $this->assertTrue($result);
            ...
            $row = DBQueryToRow("SELECT snapshot FROM uo_scoresheet_history WHERE history_id=$historyId");
    ...
    public function testPrivacyAnonymizePlayerLeavesNonSnapshotRowsAlone(): void
    {
        ...
        $historyId = $this->insertScoresheetHistorySnapshotRow(null);
        try {
            self::flushQueryCaches();
            $this->assertTrue(PrivacyAnonymizePlayer($playerId, 'admin'));
            self::flushQueryCaches();

            $row = DBQueryToRow("SELECT has_snapshot, snapshot FROM uo_scoresheet_history WHERE history_id=$historyId");
    ...
    public function testPrivacyCollectPlayerReportDataIncludesOwnScoresheetHistorySnapshotNamesButNotOtherPlayers(): void
    {
        // C1: the player export previously projected no uo_scoresheet_history data
        ...
        // PrivacyPlayerScoresheetHistoryNameRows() -- so the snapshot's OLD text is
        ...
        try {
            self::flushQueryCaches();
            $data = PrivacyCollectPlayerReportData($playerId);
            $this->assertArrayHasKey('scoresheet_history_snapshot_rows', $data);

            $names = array_column($data['scoresheet_history_snapshot_rows'], 'name');
            ...
            $scorerRows = array_values(array_filter(
                $data['scoresheet_history_snapshot_rows'],
    ...
    public function testPrivacyDeleteUserDataAnonymizesScoresheetHistoryRowsInPlaceInsteadOfDeletingThem(): void
    {
        ...
        try {
            ...
            $before = DBQueryToArray(
                "SELECT history_id, game, snapshot, detail FROM uo_scoresheet_history WHERE history_id IN ($snapshotId, $detailId) ORDER BY history_id",
            );
            ...
            $after = DBQueryToArray(
                "SELECT history_id, game, user_id, ip, snapshot, detail FROM uo_scoresheet_history WHERE history_id IN ($snapshotId, $detailId) ORDER BY history_id",
            );
            ...
            $this->cleanupScoresheetHistoryRows([$snapshotId, $detailId]);
    ...
    public function testPrivacyCollectUserReportDataExcludesSnapshotColumnButIncludesTheUsersOwnRow(): void
    {
        ...
        try {
            self::flushQueryCaches();
            $data = PrivacyCollectUserReportData('admin');
            $this->assertArrayHasKey('scoresheet_history_rows', $data);
            $found = null;
